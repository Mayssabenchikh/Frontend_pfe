...
import { XMarkIcon, ExclamationCircleIcon, CameraIcon, ArrowPathIcon } from "../../icons/heroicons/outline";
...
export function EditUserModal({
  ...
  const prevUserId = useRef<string | undefined>(_user?.uuid);
  if (_user?.uuid !== prevUserId.current) {
    prevUserId.current = _user?.uuid;
  ...
  const handleAvatarChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await http.post<{ avatarUrl: string }>(ADMIN_API_PATHS.userAvatar(_user.uuid), formData, {
