...
export function ExplanationBox({ deterministicSummary, aiExplanation, loading }: Props) {
  ...
  if (!hasAi && deterministicUnavailable) {
    return <p className="text-xs text-slate-500">Synthèse technique indisponible pour cette correspondance.</p>;
  }

  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3">
        <p className="text-sm leading-relaxed text-slate-800">{deterministicSummary}</p>
      </div>

      {hasAi ? (
        <div className="rounded-xl border border-indigo-100 bg-gradient-to-br from-indigo-50/90 to-white px-4 py-3 shadow-sm">
          <p className="text-sm text-slate-800">{aiData.summary || "Aide IA disponible."}</p>
        </div>
      ) : null}
    </div>
  );
